...
def assemble_rushing(pbp, rosters, parts, season):
    ...
    # 1 Yards before contact (need a source for this)
    # 2 Broken tackles (need a source for this)
    # 3 Forced missed tackles (need a source for this)
    # 4 RPO carry rate (need a source for this)
    # 5 Read option rate (need a source for this)
    # 6 Yards over expected per attempt (need a expected rush yards data source)
    # 7 More sophisticated EPA model (currently have naive model and PPA from CFBD)
    ...
    out = pd.DataFrame({
        ...
        'fumbles': g['fumble'].sum() if 'fumble' in df.columns else 0,
        'fumbles_lost': g['fumble_lost'].sum() if 'fumble_lost' in df.columns else 0,
        'ppa_total_rush': g['ppa_connelly'].sum()
    ...
    out['ppa_per_rush'] = out['ppa_total_rush'].div(out['rush_att'].replace({0: np.nan}))
    ...
    out['lost_fumble_rate'] = out['fumbles_lost'].div(out['rush_att'].replace({0: np.nan}))
    ...
    cols = ['season','player_id','player_name','team_id','team_name','conference','position',
            ...
            'success_rate','explosive_rush_rate',
